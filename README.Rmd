...
motifr currently can handle unweighted directed and undirected networks. There is in principle no restriction on the number of levels of a network to count motifs in, but typical use cases for which the package was designed will likely involve two- or three-level networks.
Support for undirected networks has only been introduced recently. Hence, most supported motifs are undirected.

Network data should currently be prepared as statnet network objects with a numeric vertex attribute to specify a level for each node (named e.g. "lvl") for best results.
...
``identify_gaps`` has a sibling in ``critical_dyads``. Critical_dyads works in reverse to identifying gaps - it analyses for every existing edge how many instances of a given motif would disappear if the edge would be removed.

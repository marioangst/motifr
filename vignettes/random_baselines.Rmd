...
vignette: >
  %\VignetteIndexEntry{Random baselines}
...
- *Partial Exponential Random Graph Model* (``partial_ergm``) It tends to be hard fit an ERGM to a multi-level network. However, this can be often avoided by using ERGM only for modelling the behaviour of one level while keeping the other levels fixed.
